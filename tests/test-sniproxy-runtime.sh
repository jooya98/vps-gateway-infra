#!/usr/bin/env bash
set -euo pipefail
ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd); TMP=$(mktemp -d); trap '[[ -n "${PID:-}" ]] && kill "$PID" 2>/dev/null || true; rm -rf "$TMP"' EXIT
...
install -d -m 0755 /etc/sniproxy
install -m 0644 "$TMP/rendered/sniproxy/config.yaml" /etc/sniproxy/config.yaml
install -m 0644 "$TMP/rendered/sniproxy/domains.csv" /etc/sniproxy/domains.csv
install -m 0644 "$TMP/rendered/sniproxy/cidr.csv" /etc/sniproxy/cidr.csv
sniproxy --config /etc/sniproxy/config.yaml > "$TMP/sniproxy.log" 2>&1 & PID=$!
sleep 2
kill -0 "$PID"
selected=$(dig +short @127.0.0.1 openai.com A | tail -1); [[ "$selected" == 127.0.0.1 ]]
normal=$(dig +short @127.0.0.1 example.com A | head -1); [[ -n "$normal" && "$normal" != 127.0.0.1 ]]
code=$(curl -sS -o /dev/null -w '%{http_code}' --connect-timeout 8 --max-time 12 --resolve openai.com:443:127.0.0.1 https://openai.com/ || true)
[[ "$code" =~ ^[0-9]{3}$ ]] || { echo "selected HTTPS did not produce an HTTP response; sniproxy log:"; cat "$TMP/sniproxy.log"; exit 1; }
