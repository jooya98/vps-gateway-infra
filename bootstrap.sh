...
if [[ -f "$ROOT/config/packages.env" ]]; then
  ...
  if [[ "$TEST_MODE" == 1 ]]; then printf '%s\n' 'bootstrap: base OS package installation skipped (test mode)'; else "$BASE_BOOTSTRAP_SCRIPT"; fi
fi
if [[ "$TEST_MODE" != 1 ]]; then
...
if [[ ! -x "$SING_BOX_BIN" ]]; then printf '%s\n' 'bootstrap: installing official sing-box'; "$SING_BOX_INSTALL_SCRIPT"; fi
...
prompt_value(){ local __var=$1 label=$2 default=${3:-} value; if [[ -n "$default" ]]; then read -r -p "$label [$default]: " value; value=${value:-$default}; else read -r -p "$label: " value; fi; [[ -n "$value" ]] || fail "$__var is required"; printf -v "$__var" '%s' "$value"; }
prompt_secret(){ local __var=$1 label=$2 default=${3:-} value; if [[ -n "$default" ]]; then read -r -s -p "$label [saved; Enter to reuse]: " value; printf '\n'; value=${value:-$default}; else read -r -s -p "$label: " value; printf '\n'; fi; [[ -n "$value" ]] || fail "$__var is required"; printf -v "$__var" '%s' "$value"; }
if [[ ! -f "$RUNTIME_FILE" ]]; then
  SOCKS_USERNAME=${SOCKS_USERNAME:-gateway}; [[ "$TEST_MODE" == 1 ]] || prompt_value SOCKS_USERNAME 'SOCKS/HTTP username' "$SOCKS_USERNAME"
  RUNTIME_FILE="$RUNTIME_FILE" CLIENT_INFO_FILE="$CLIENT_INFO_FILE" SING_BOX_BIN="$SING_BOX_BIN" SOCKS_USERNAME="$SOCKS_USERNAME" SERVER_ADDRESS='' "$GENERATE_SCRIPT"
else
...
for spec in 'CLOUDFLARE_API_TOKEN|Cloudflare API token|' 'CLOUDFLARE_ACCOUNT_ID|Cloudflare account ID|' 'CLOUDFLARE_ZONE_NAME|Cloudflare zone|engine.qzz.io' 'PUBLIC_HOSTNAME|Tunnel public hostname|echo.engine.qzz.io' 'DIRECT_HOSTNAME|Direct TLS hostname|direct.echo.engine.qzz.io' 'CLOUDFLARE_TUNNEL_NAME|Cloudflare Tunnel name|echo-gateway' 'CERTBOT_EMAIL|Let’s Encrypt email|'; do
  IFS='|' read -r name label default <<< "$spec"; current=$(awk -F= -v n="$name" '$1==n{$1="";sub(/^=/,"");print;exit}' "$RUNTIME_FILE" 2>/dev/null || true); [[ -n "$current" ]] && continue; [[ "$TEST_MODE" == 1 ]] && continue
  case "$name" in CLOUDFLARE_API_TOKEN) prompt_secret "$name" "$label" '';; CERTBOT_EMAIL) prompt_value "$name" "$label" '';; *) prompt_value "$name" "$label" "$default";; esac
...
[[ "$PUBLIC_HOSTNAME" != "$DIRECT_HOSTNAME" ]] || fail 'PUBLIC_HOSTNAME and DIRECT_HOSTNAME must be different'
...
PROFILE="$PROFILE" RUNTIME_FILE="$RUNTIME_FILE" CLIENT_INFO_FILE="$CLIENT_INFO_FILE" "$DEPLOY_SCRIPT" --env-file "$RUNTIME_FILE"
