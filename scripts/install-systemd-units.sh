...
install -m 0600 "$OUT_DIR/sing-box/config.json" "$SING_BOX_CONFIG_PATH"
...
if [[ "${ENABLE_CLOUDFLARED:-false}" == true ]]; then
  ...
  printf '%s' "$CLOUDFLARED_TUNNEL_TOKEN" > "$CLOUDFLARED_TOKEN_PATH"
  chmod 0600 "$CLOUDFLARED_TOKEN_PATH"
...
if [[ "${ENABLE_DNS_STEERING:-false}" == true ]]; then
  ...
  install -m 0644 "$OUT_DIR/sniproxy/config.yaml" "$SNIPROXY_CONFIG_PATH"
  install -m 0644 "$OUT_DIR/sniproxy/domains.csv" "$SNIPROXY_DOMAINS_PATH"
  install -m 0644 "$OUT_DIR/sniproxy/cidr.csv" "$SNIPROXY_CIDR_PATH"
  install -m 0644 "$ROOT/templates/sniproxy/sniproxy.service.tmpl" /dev/null 2>/dev/null || true
...
install -m 0644 "$OUT_DIR/ssh/00-vps-gateway-hardening.conf" /etc/ssh/sshd_config.d/00-vps-gateway-hardening.conf
