...
# This script is deliberately NOT integrated into the deploy flow.
# Run it manually after bootstrap, or invoke from an external orchestration step.

set -euo pipefail

# ---------------------------------------------------------------------------
# Preconditions – must be run as root
if [[ "$(id -u)" != "0" ]]; then
...
# ---------------------------------------------------------------------------
# Prompt for password only for new users.
# Password is created for recovery/console use only.
# SSH password authentication is disabled by ssh hardening.
...
# ---------------------------------------------------------------------------
# Create passwordless sudoers file
