...
EOF
# The resilient renderer only needs the key to be syntactically present here because
# this test is focused on sniproxy. sing-box validity is covered by test-resilient.sh.
...
sniproxy --config /etc/sniproxy/config.yaml > "$TMP/sniproxy.log" 2>&1 & PID=$!; sleep 2
