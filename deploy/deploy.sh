...
if [[ "$DRY_RUN" == 0 ]]; then
  ...
  PROFILE="$PROFILE" RUNTIME_FILE="$ENV_FILE" "$ROOT/scripts/activate-cloudflared-local.sh"
  ...
  if [[ -x "$CLOUDFLARED_BIN" ]]; then
    PROFILE="$PROFILE" RUNTIME_FILE="$ENV_FILE" DRY_RUN=1 "$ROOT/scripts/activate-cloudflared-local.sh"
  fi
