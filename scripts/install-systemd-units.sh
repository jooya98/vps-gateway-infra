...
PROFILE=${PROFILE:-default}; ENV_FILE=${ENV_FILE:-"$ROOT/.env"}; DRY_RUN=${DRY_RUN:-0}
...
if [[ "$ENABLE_DNS_STEERING" == true ]]; then
 install -d -m 0755 /etc/sniproxy; install -m 0644 "$OUT_DIR/sniproxy/config.yaml" "$SNIPROXY_CONFIG_PATH"; install -m 0644 "$OUT_DIR/sniproxy/domains.csv" "$SNIPROXY_DOMAINS_PATH"; install -m 0644 "$OUT_DIR/sniproxy/cidr.csv" "$SNIPROXY_CIDR_PATH"
